...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        # not really a bias but more of a mask, but following the OpenAI/HF naming
        self.register_buffer("bias", torch.tril(torch.ones(config.block_size, config.block_size)).view(1,1, config.block_size,config.block_size))
        self.c_proj.NANOGPT_SCALE_INIT = 1.0
    def forward(self, x):
        B, T, C = x.size() # batch size, sequence length, embedding dimensionality(n_embd) 
        #calulate query, key, values for all heads in a batch and move head forward to the batch
        # nh is "number of heads", hs is the head size and C (no of channels) = nh *  hs
        #eg in gpt 2 124m version, n_head=12, hs=64, so nh*hs=C=768 channels in the transformer
        qkv = self.c_attn(x)
        q, k, v = qkv.split(self.n_embd, dim=2)
        k = k.view(B, T, self.n_head, C//self.n_head).transpose(1,2) # (B, nh, T, hs)
        q = q.view(B, T, self.n_head, C//self.n_head).transpose(1,2)
        v = v.view(B, T, self.n_head, C//self.n_head).transpose(1,2)
        # attention (materializes the large (T, T) matrix for all the queries and the keys)
        ...
        # replace with flash attention (fused kernel approach to speeding up softmax)
...
class GPT(nn.Module):
    ...
    def configure_optimizers(self,weight_decay, learning_rate, device):
        ...
        # ie all the weight tensors in matmuls and embeddings decay, all biases and layer norms dont
    ...
    def forward(self, idx, targets=None):
        ...
        x = tok_emb + pos_emb
        # forward the bloacks of the transformer
...
class DataLoaderLite:
    ...
    def next_batch(self):
        ...
        if self.current_position + (B * T * self.num_processes
+ 1) > len(self.tokens):
...
# torchrun command sets the env varibles RANK, LOCAL_RANK AND WORLD_SIZE
ddp = int(os.environ.get("RANK", -1)) != -1 # is this a ddp run?
if ddp:
    # use of DDP demands CUDA, we set the device appropriately according to rank
    assert torch.cuda.is_available(), "I guess we need cuda for DDP"
    ...
    master_process = ddp_rank == 0 # the process that will do logging, checkpointing etc.
else:
    # vanilla, non DDP run
    ddp_rank =0
    ddp_local_rank=0
...
total_batch_size = 491520 # divisible by B*T, ~0.48M tokens
B=16 # micro batch size - leaving ~2GB headroom to avoid OOM spikes
...
def get_lr(it):
    ...
    # if it > lr_decay_iters, return the min learning rate
    if it > max_steps:
        return min_lr
...
log_file = os.path.join(log_dir, "log_1.txt")
with open(log_file, "w") as f: # open for writing to clear the file
    pass

# checkpoint directory
checkpoint_dir = "log/checkpoints/v2"
...
for step in range(max_steps):
    ...
    model.train()
    optimizer.zero_grad()
    loss_accum = torch.tensor(0.0, requires_grad=False,device=device)
    ...
    if master_process:
        print(f"step {step}| loss:{loss_accum.item():.4f}| lr: {lr:.4e} | norm: {norm:.4f}  dt:{dt:.2f}| tok/sec: {tokens_per_sec:.4f} ")
        with open(log_file, "a") as f:
            f.write(f"step {step}| loss:{loss_accum.item():.4f}| lr: {lr:.4e} | norm: {norm:.4f}  dt:{dt:.2f}| tok/sec: {tokens_per_sec:.4f}\n")
        wandb.log({
